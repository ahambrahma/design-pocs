...
func NewDynamoDB() *DynamoDB {
	...
	return &DynamoDB{
		dbConn:             db,
		tableKeyAttributes: make(map[string]KeyAttributes),
	}
}
...
func (d *DynamoDB) CreateTable(input CreateTableInput) error {
	...
	d.tableKeyAttributes[tableName] = input.Keys
	return nil
}
...
func (d *DynamoDB) PutItem(input PutItemInput) error {
	...
	value, ok := d.tableKeyAttributes[tableName]
	if !ok {
		return fmt.Errorf("table %s does not exist", tableName)
	}

	partitionKeyAttr := value.PartitionKey
	sortKeyAttr := value.SortKey

	if input.KeyAttributes[partitionKeyAttr] == "" {
		return fmt.Errorf("partition key %s is required", partitionKeyAttr)
	}

	if sortKeyAttr != nil && input.KeyAttributes[*sortKeyAttr] == "" {
	...
	if sortKeyAttr != nil {
		columns = append(columns, *sortKeyAttr)
		placeholders = append(placeholders, "?")
		args = append(args, input.KeyAttributes[*sortKeyAttr])
	}
...
func (d *DynamoDB) GetItem(input GetItemInput) (*GetItemOutput, error) {
	...
	partitionKeyAttr := keyAttrs.PartitionKey
	sortKeyAttr := keyAttrs.SortKey
	...
	if row == nil || row.Err() != nil {
		fmt.Printf("Failed to get item from table %s: %v\n", tableName)
