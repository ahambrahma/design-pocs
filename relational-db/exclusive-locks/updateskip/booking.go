...
type SeatBookingWithUpdateSkip struct {
}

func (s *SeatBookingWithUpdateSkip) BookSeats() {
	start := time.Now()
	...
	wg.Wait()
	fmt.Printf("Total time taken with UPDATE SKIP lock: %v seconds\n", time.Since(start).Seconds())
}

func (s *SeatBookingWithUpdateSkip) bookSeat(wg *sync.WaitGroup, userID int) error {
	defer wg.Done()
	...
	row := txn.QueryRow("SELECT id, number, user_id FROM seats WHERE user_id IS NULL LIMIT 1 FOR UPDATE SKIP LOCKED")
