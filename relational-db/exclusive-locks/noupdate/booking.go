...
type SeatBookingWithoutUpdate struct {
}

func (s *SeatBookingWithoutUpdate) BookSeats() {
	start := time.Now()
	...
	wg.Wait()
	fmt.Printf("Total time taken without UPDATE lock: %v seconds\n", time.Since(start).Seconds())
}

func (s *SeatBookingWithoutUpdate) bookSeat(wg *sync.WaitGroup, userID int) error {
	defer wg.Done()
	...
	row := txn.QueryRow("SELECT id, number, user_id FROM seats WHERE user_id IS NULL LIMIT 1")
